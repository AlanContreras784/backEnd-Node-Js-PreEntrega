...
const [metodo, valor, ...items] = args;
// const metodo = args[0]; // GET, POST, PUT, PATCH, DELETE
// const valor = args[1]; // {data} o {id}
...
switch (metodo.toUpperCase()) {
    case "GET":
        if (!valor) {
            ...
            const productos = await obtenerProductos(valor);
    ...
    case "POST":
        if (!valor) {
            ...
            const nuevoProducto = await agregarProducto(valor, items[0], items[1], items[2]);
    ...
    case "PUT":
        if (!valor) {
            ...
            const productoModificado = await modificarProducto(valor, items[0], items[1], items[2]);
    ...
    case "DELETE":
        if (!valor) {
            ...
            const productoEliminado =  await eliminarProducto(valor);
